...
LOG_INFO "found master ip by ENV key \"MASTER_IP\": ${MASTER_IP}"
mv /etc/bind/named.conf.options.slave /etc/bind/named.conf.options
sed -i "s/MASTER_IP/$MASTER_IP/" /etc/bind/named.conf.options
